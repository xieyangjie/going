...
func NewEmail(subject string, content string, contentType string) *Email {
	var email = &Email{Headers: textproto.MIMEHeader{}}
...
func NewTextEmail(subject string, content string) *Email {
	return NewEmail(subject, content, "text/plain")
}

func NewHtmlEmail(subject string, content string) *Email {
	return NewEmail(subject, content, "text/html")
}
...
func (this *Email) Send(config *MailConfig) error {
	if config == nil {
		return errors.New("config 不能为空")
	}
	if this.from != config.Username {
